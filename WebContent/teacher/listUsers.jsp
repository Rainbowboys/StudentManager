		...
		<div class="row-fluid">
			...
			<div class="col-md-10 ">
				<table class="table table-striped">
					...
					<c:forEach items="${requestScope.teacherBeans}" var="item" varStatus="status">
						<tr>
							<td>${item.id}</td>
							<td><a>${item.name}</a></td>
							...
							<td>${item.password}</td>
							<td><a
								href="${pageContext.request.contextPath}/teacher/TeacherServlet?method=toupdate&id=${item.id}">修改</a></td>

							<td><a
								href="${pageContext.request.contextPath}/teacher/TeacherServlet?method=delete&id=${item.id}">删除</a></td>
						</tr>
					</c:forEach>
				</table>
				<c:if test="${status=='2'}">
